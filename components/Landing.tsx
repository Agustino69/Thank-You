...
const Landing: React.FC<LandingProps> = ({ 
  ...
  const ambientSfxRef = useRef<HTMLAudioElement>(null); // Computer fan/hum
  const roomSfxRef = useRef<HTMLAudioElement>(null); // External room ambience
  const beepSfxRef = useRef<HTMLAudioElement>(null);
  const occasionalSfxRef = useRef<HTMLAudioElement>(null);

  // Refs for timeouts to clear them on power off
  const bootTimeoutsRef = useRef<ReturnType<typeof setTimeout>[]>([]);
  const occasionalTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const togglePower = async () => {
      ...
      if (nextState) {
          ...
          // Play Boot Sound immediately on user interaction
          if (bootSfxRef.current && systemConfig.bootSfxUrl) {
              bootSfxRef.current.volume = 0.3;
              bootSfxRef.current.currentTime = 0;
              try {
          ...
          // TURNING OFF
          // Stop terminal specific audio, BUT KEEP ROOM AUDIO
          if (bootSfxRef.current) { bootSfxRef.current.pause(); bootSfxRef.current.currentTime = 0; }
          if (ambientSfxRef.current) { ambientSfxRef.current.pause(); ambientSfxRef.current.currentTime = 0; }
          if (occasionalSfxRef.current) { occasionalSfxRef.current.pause(); }
          if (audioEggRef.current) { audioEggRef.current.pause(); }

          // Clear timeouts
          bootTimeoutsRef.current.forEach(clearTimeout);
          bootTimeoutsRef.current = [];
          if (occasionalTimeoutRef.current) clearTimeout(occasionalTimeoutRef.current);
  ...
  // Handle Boot Sound Ending -> Start Terminal Ambient
  const handleBootEnded = () => {
      // Only start terminal ambient if we are still powered on and not unlocking
      if (isPowered && ambientSfxRef.current && systemConfig.ambientSfxUrl && !isUnlocking) {
          ambientSfxRef.current.volume = 0.05; // Slightly audible hum
          ambientSfxRef.current.play().catch(e => console.error("Ambient play failed", e));
      }
  };

  // Global Click to focus input AND start ROOM AMBIENCE
  useEffect(() => {
    const handleGlobalClick = () => {
        // 1. Focus Logic
        if (isPowered && !isUnlocking && !isBooting) {
            inputRef.current?.focus();
        }

        // 2. Room Ambience Logic (Plays regardless of power, starts on interaction)
        if (roomSfxRef.current && roomSfxRef.current.paused && systemConfig.roomSfxUrl && !isUnlocking) {
            roomSfxRef.current.volume = 0.1; // Subtle room noise
            roomSfxRef.current.play().catch(() => { /* Auto-play blocked handled silently */ });
        }
    };
    document.addEventListener('click', handleGlobalClick);
    return () => document.removeEventListener('click', handleGlobalClick);
  }, [isUnlocking, isBooting, isPowered, systemConfig.roomSfxUrl]);
  ...
  // ------------------------------------------------
  useEffect(() => {
    if (!isPowered || isBooting || isUnlocking || !systemConfig.occasionalSfxUrl) {
        if (occasionalTimeoutRef.current) clearTimeout(occasionalTimeoutRef.current);
        return;
    }
    ...
    const scheduleSound = () => {
        ...
        occasionalTimeoutRef.current = setTimeout(() => {
            if (occasionalSfxRef.current && isPowered) {
  ...
  }, [isBooting, isUnlocking, systemConfig.occasionalSfxUrl, isPowered]);

  // Pause all ambient sounds on unlock
  useEffect(() => {
    if (isUnlocking) {
        if (ambientSfxRef.current) ambientSfxRef.current.pause();
        // Also pause room ambience when transitioning to content
        if (roomSfxRef.current) {
             // Optional: fade out logic could go here, but pause is immediate
             roomSfxRef.current.pause();
        }
    }
  }, [isUnlocking]);
  ...
  useEffect(() => {
    ...
    if (activeEasterEgg && activeEasterEgg.type === 'audio' && activeEasterEgg.response) {
        ...
        // Resume terminal ambient if egg cleared
        if (isPowered && !isUnlocking && !isBooting && ambientSfxRef.current && ambientSfxRef.current.paused && systemConfig.ambientSfxUrl) {
  ...
  }, [activeEasterEgg, isBooting, isUnlocking, isPowered, systemConfig.ambientSfxUrl]);
  ...
  return (
    ...
    >
      <style>{`
        .font-workbench { font-variation-settings: "BLED" 0, "SCAN" -3; }
        .expanded-text { transform: scale(1.05, 0.9); transform-origin: center; width: 95%; margin: 0 auto; }
        .scanlines {
        ...
        .color-bleed { text-shadow: -0.5px 0 rgba(255,0,0,0.4), 0.5px 0 rgba(0,255,255,0.4), 0 0 2px rgba(255, 255, 255, 0.1); }
        .terminal-screen { box-shadow: inset 0 0 100px rgba(0,0,0,0.95); background-color: #050505; }
        .glitch-text { position: relative; display: inline-block; }
        .glitch-text::before, .glitch-text::after { content: attr(data-text); position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: #050505; opacity: 0.8; }
        ...
        @keyframes glitch-anim { 0% { clip: rect(2px, 9999px, 5px, 0); } 5% { clip: rect(65px, 9999px, 100px, 0); } 100% { clip: rect(2px, 9999px, 5px, 0); } }
        @keyframes glitch-anim2 { 0% { clip: rect(65px, 9999px, 100px, 0); } 5% { clip: rect(2px, 9999px, 5px, 0); } 100% { clip: rect(65px, 9999px, 100px, 0); } }
      `}</style>
      ...
      {/* 2. Computer Hum (Power dependent) */}
      ...
      {/* 3. Room Ambience (Always on after interaction) */}
      {systemConfig.roomSfxUrl && <audio ref={roomSfxRef} src={getAudioUrl(systemConfig.roomSfxUrl)} loop preload="auto" className="hidden" />}
      ...
      {/* TERMINAL CONTAINER (REVERTED TO ORIGINAL STYLE) */}
      <motion.div 
      ...
      >
          ...
          {/* CRT ON/OFF ANIMATION WRAPPER */}
          <AnimatePresence>
             {isPowered && (
                 <motion.div
                    initial={{ scaleY: 0.005, scaleX: 0, opacity: 0.5, filter: 'brightness(5)' }}
                    animate={{ 
      ...
      {/* EXTERNAL POWER BUTTON (Centered below terminal) */}
      <div className="mt-8 z-50">
          <button 
              onClick={togglePower}
              className={`relative w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all duration-300 shadow-lg active:scale-95 ${
                  isPowered 
                  ? 'bg-zinc-800 border-zinc-700 shadow-[0_0_15px_rgba(0,255,0,0.3)]' 
                  : 'bg-zinc-900 border-zinc-800'
              }`}
          >
              <Power size={18} className={`transition-all duration-500 ${isPowered ? 'text-green-500 drop-shadow-[0_0_5px_rgba(0,255,0,0.8)]' : 'text-zinc-600'}`} />
              {/* LED Indicator */}
              <div className={`absolute -bottom-3 w-1.5 h-1.5 rounded-full transition-all duration-500 ${isPowered ? 'bg-green-500 shadow-[0_0_8px_#22c55e]' : 'bg-red-900'}`}></div>
          </button>
      </div>
